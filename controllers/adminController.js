...
class AdminController {
    ...
    async testDeleteChat(req, res){
        const {id} = req.body;
        try{        
            await adminLayer.deleteChat(id)
            return res.status(200).json({ message: "Чат удален" });
            // return res.json(listChat);
        }catch(e){
            console.log();
        }
    ...
    async testSelectChat(req, res){
        // // console.log(req.body.id);
        // console.log("ТЫ ТУТУ!!!!", req);
        // const {id} = req.body;
        // console.log(req.body);
        try{
            const chat = await adminLayer.selectChat();
            // console.log(chat);
